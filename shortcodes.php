...
function shortcodes_page(){
	...
	<div class="wrap">
		...
		<ol>
			...
			<li>[subscribe-form class=""] <span class="sdetagils">displays Subcribe Form</span></li>	
		</ol>
	</div>
	<?php
}
...
function subscribe_form_func($atts = array(), $content = '') {
	$atts = shortcode_atts( array(
        'class' => '',
	), $atts, 'subscribe-form' );
    ob_start(); ?>
        
        <form id="email-form" name="email-form" data-name="Email Form" class="subscribe-v3-form" _lpchecked="1">
            <div class="subscribe-v3-text">รับอีเมล์สรุปบทความจาก Designil รายสัปดาห์ผ่านทาง Inbox</div>
            <div class="w-layout-grid subscribe-v3-grid"><input type="email" class="input w-input" maxlength="256" name="email-2" data-name="Email 2" placeholder="Email address" id="email-2" required=""><input type="submit" value="Subscribe" data-wait="Please wait..." class="button w-button"></div>
            <div class="form-info">ไม่มีการแสปมแน่นอน 100% ข้อมูลของคุณจะถูกใช้สำหรับส่งบทความ ลิงค์ดี ๆ จาก Designil เท่านั้นครับ.</div>
        </form>
        
    <?php $html = ob_get_clean();
    return $html;
}
add_shortcode( 'subscribe-form', 'subscribe_form_func' );